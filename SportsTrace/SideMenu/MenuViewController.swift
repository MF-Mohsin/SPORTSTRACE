...
class MenuViewController: UIViewController {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        let sideMenuBasicConfiguration = SideMenuController.preferences.basic
        let showPlaceTableOnLeft = (sideMenuBasicConfiguration.position == .under) != (sideMenuBasicConfiguration.direction == .right)
        selectionMenuTrailingConstraint.constant = showPlaceTableOnLeft ? SideMenuController.preferences.basic.menuWidth : 0
        view.layoutIfNeeded()
    }
}
...
extension MenuViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
     ...
     //   cell.contentView.backgroundColor = themeColor
        ...
        if row == 0 {
            cell.titleLabel?.text = "Sessions"
        } else if row == 1 {
            cell.titleLabel?.text = "Analytics"
        } else if row == 2 {
            cell.titleLabel?.text = "Recomandations"
        }
